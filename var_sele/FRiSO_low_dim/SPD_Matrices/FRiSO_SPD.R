...
foo <- function(irep){
  ...
  wutest=gendata_SPD(n,p,m,r,seed) # same test set for different reps
  xtest=wutest$x
  Mintest=wutest$Min
  ...
  train_data <- gendata_SPD(n, p, m, r, irep)
  x <- train_data$x
  Min <- train_data$Min
  dc_start <- Sys.time()
  dcov <- compute_dcov2(x, Min, "SPD", cholesky_error_C)
  ...
  # fitting time if we select 9 variables 
  ind <- dcov_ordered[1:9]
  ind <- ind[order(ind)]
  fit_start <- Sys.time()
  frechet_fit <- GloCovReg(x=as.matrix(x[, ind]), M = Min, xout = as.matrix(x[, ind]),
                           optns = list(metric="cholesky"))
  ...
  frechet_fit <- GloCovReg(x=as.matrix(x[, ind]), M = Min, xout = as.matrix(x[, ind]),
                           optns = list(metric="cholesky"))
  fit_time3 <- difftime(Sys.time(), fit_start, units = "secs")
  ...
  result=list(hopt=hopt,                                      # all lambdas given tau
              ...
              fit_time2 = fit_time2, fit_time3 = fit_time3) 
...
friso_SPD=foreach(irep=1:100) %dopar% foo(irep)

save(file="friso_SPD.Rdata", friso_SPD)
