...
foo <- function(irep){
  ...
  wutest=gendata_SPD_cate(n,p,m,r,seed) # same test set for different reps
  xtest=wutest$x
  Mintest=wutest$Min
  ...
  train_data <- gendata_SPD_cate(n, p, m, r, irep)
  x <- train_data$x
  Min <- train_data$Min
  dc_start <- Sys.time()
  dcov <- compute_dcov2(x, Min, "cate", cholesky_error_C)
...
friso_cate=foreach(irep=1:100) %dopar% foo(irep)

save(file="friso_cate.Rdata", friso_cate)
