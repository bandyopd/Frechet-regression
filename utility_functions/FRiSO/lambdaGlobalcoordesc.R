...
#' Frechet regression when the output is an SPD matrix equipped with the Cholesky decomposition distance
#' 
#' @details For algorithm details, see section 1 of the supplementary materials
#' 
#' @param x: n by p matrix of predictors
#' @param Min: An m by m SPD matrix 
...
#' 
lambdaGlobalcoordesc=function(x,Min, tau, lambdainit=NULL) {
  p=ncol(x)
  ...
  curobj=GlobalObj4h(x=x,Min=Min,lambda= lambdacur)
  ...
  while(wu ){
    ...
    for (j in 1:p){
      ...
      if(sum(lambdacurMj)>eps0) {
        ...
        # Make only jth lambda important
        lambdaj=0*lambdacur
        lambdaj[j]=1
        
        # individually penalized ridge Frechet regression to optimized over weight ttt in (0,1)
        fffun<-function(ttt){
          return(GlobalObj4h(x,Min, ((lambdaj-lambdacurMj)*ttt+lambdacurMj)*tau))
        }
