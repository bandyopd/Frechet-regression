...
source(paste0(file_folder1, "spatial_smooth.R"))
file_folder2 <- "/"
# Rcpp::sourceCpp("IndivRidgeGFRCovCholesky_vC.cpp")
Rcpp::sourceCpp(paste0(file_folder2, "dcov_rcpp.cpp"))
Rcpp::sourceCpp(paste0(file_folder2, "spatial_smooth_cpp.cpp"))
library(gglasso)
library(mgcv)
library(geoR)
...
x <- gendata_SPD(200, 3000, 3, 0.5, 2023)$x
Y <- gen_SWP(x, 15, 20, 3, 2023)
...
n_vox <- dim(Y)[4]
pe_SPD_p3000 = foreach(irep=1:n_vox) %dopar% foo(irep)

save(pe_SPD_p3000, Y, file="pe_SPD_p3000.Rdata")
